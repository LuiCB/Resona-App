...
def resample_audio(
    ...
    # Option B: numpy linear interpolation (lightweight, no torchaudio needed)
...
def get_vad_chunks(waveform: np.ndarray, sr: int) -> list[tuple[int, int]]:
    ...
    """
    # Energy-based VAD (lightweight, no torch/silero dependency)
    frame_len = int(sr * 0.03)  # 30ms frames
    hop = frame_len
    energy = np.array([
        np.sqrt(np.mean(waveform[i:i+frame_len]**2))
    ...
    threshold = max(np.median(energy) * 1.5, 1e-4)
    ...
    for i, s in enumerate(is_speech):
        if s and not in_speech:
            ...
            # Merge short gaps
            if segments and (start - segments[-1][1]) < int(sr * VAD_MIN_SILENCE_MS / 1000):
